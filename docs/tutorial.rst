...
The goal of this tutorial is to give you a quick introduction to MooQuant.
As described in the introduction, the goal of MooQuant is to help you backtest stock trading strategies.
...
then MooQuant should allow you to do so with minimal effort.
...
MooQuant has 6 main components:
...
Let's use Oracle's stock prices for year 2000, which we'll download with the following command: ::
    ...
    python -c "from mooquant.tools import yahoofinance; yahoofinance.download_daily_bars('orcl', 2000, 'orcl-2000.csv')"

The mooquant.tools.yahoofinance package downloads CSV formatted data from Yahoo! Finance. 
...
Let's start by downloading 3 years of daily bars for 'Dow Jones Industrial Average': ::
    ...
    python -c "from mooquant.tools import yahoofinance; yahoofinance.download_daily_bars('dia', 2009, 'dia-2009.csv')"
    python -c "from mooquant.tools import yahoofinance; yahoofinance.download_daily_bars('dia', 2010, 'dia-2010.csv')" 
    python -c "from mooquant.tools import yahoofinance; yahoofinance.download_daily_bars('dia', 2011, 'dia-2011.csv')"
...
This is the worker script that uses the **mooquant.optimizer.worker** module to run the strategy in parallel with
...
If you just want to run strategies in parallel in your own desktop you can take advantage of the **mooquant.optimizer.local**
module like this:
...
The code is doing 3 things:
 ...
 2. Loading the feed with the CSV files we downloaded.
 3. Using the **mooquant.optimizer.local** module to run the strategy in parallel and find the best result.
...
MooQuant makes it very easy to plot a strategy execution.
...
I hope you enjoyed this quick introduction. I'd recommend you to download MooQuant here: http://gbeced.github.io/mooquant/downloads/index.html 
