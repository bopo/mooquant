...
The key things to highlight are:
 ...
 1. We're using :class:`mooquant.strategy.BaseStrategy` instead of :class:`mooquant.strategy.BacktestingStrategy`
    as the base class. This is not a backtest.
 2. Trade events get notified via the call to **onBars**. No need to manually subscribe.
 3. Order book update events are handled by manually subscribing to :class:`mooquant.bitstamp.barfeed.LiveTradeFeed.getOrderBookUpdateEvent`.
...
In order to live trade this strategy you should use :class:`mooquant.bitstamp.broker.LiveBroker` instead of
:class:`mooquant.bitstamp.broker.PaperTradingBroker`.
