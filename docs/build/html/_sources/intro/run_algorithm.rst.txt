...
在设计 MooQuant API 的时候，考虑到以上随时变化的需求，将这部分需要以参数方式的配置严格从代码层面剥离。同一份策略代码，通过启动策略时传入不同的参数来实现完全不同的策略开发、风控、运行和调优等的功能。
...
在命令行模式中，我们预先定义了常用的参数作为命令行的 option，您可以直接在控制台输入参数来配置 MooQuant，但并不是所有的参数都可以通过命令行来配置，如果有一些特殊的参数需要配置，请结合其他方式来配置您的策略。当然您也可以扩展命令行，来实现您指定的命令行 option 选项。
...
N/A           `- -` resume                    在模拟交易和实盘交易中，MooQuant支持策略的pause && resume，该选项表示开启 resume 功能
...
.. code-block:: python3
   ...
   mooquant run -rt p -fq 1m -f strategy.py --account stock 100000 -mc sys_stock_realtime.enabled True -mc sys_stock_realtime.fps 60
...
.. code-block:: python
    ...
    import click
    from mooquant import cli

接下来我们命令 :code:`mooquant run` 中添加参数 :code:`--plot` 来实现画图的功能
...
我们还希望可以通过 :code:`$ mooquant plot result_pickle_file_path` 来将之前通过pickle文件报错的某次回测的结果进行画图
...
MooQuant 在运行策略时候会在当前目录下寻找 `config.yml` 或者  `config.json` 文件作为用户配置文件来读取。
...
当创建好 `config.yml` 文件后，执行 :code:`$ mooquant run` 即可运行策略。

创建默认配置文件模板
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

您可以通过该命令在当前目录下创建一份包含了 MooQuant 基础配置项的全部参数默认值的模板文件。
...
::
    ...
    # see more config
    # http://mooquant.readthedocs.io/zh_CN/stable/intro/run_algorithm.html
    version: 0.1.6
    ...
    base:
      ...
      # Benchmark，如果不设置，默认没有基准参照。
      benchmark: ~
      # 在模拟交易和实盘交易中，MooQuant支持策略的pause && resume，该选项表示开启 resume 功能
      resume_mode: false
      # 在模拟交易和实盘交易中，MooQuant支持策略的pause && resume，该选项表示开启 persist 功能呢，
...
MooQuant 会自动识别策略中的 :code:`__config__` 变量。

.. warning::

    虽然 MooQuant 提供了此种方式来配置策略，但主要用于自动化测试中对每个策略进行参数配置，不建议在策略开发和运行中使用此方式运行策略。

通过引用 MooQuant 库在代码中运行策略
------------------------------------------------------

并不是所有业务场景下都需要使用 :code:`mooquant run` 命令行的方式来运行策略，您也可以在您的脚本/程序中直接运行 MooQuant。

.. note::

  即使通过代码方式启动策略，MooQuant 也会寻找代码执行目录是否存在 `config.yml` / `config.json` 文件，作为用户配置文件来加载配置。但代码中传入的 `config` 优先级更高。
