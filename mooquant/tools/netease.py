...
def download_daily_bars(instrument, year, csvFile):
    ...
    bars = download_json(instrument, datetime.date(year, 1, 1), datetime.date(year, 12, 31), "d")
...
def download_weekly_bars(instrument, year, csvFile):
    ...
    bars = download_json(instrument, begin, end, "w")

    with open(csvFile, "w") as f:
        f.write(bars)


def build_feed(instruments, fromDate, toDate, storage, frequency=bar.Frequency.MINUTE, timezone=None, skipErrors=False):
    ...
    logger = mooquant.logger.getLogger("netease")
    ...
    for year in range(fromYear, toYear + 1):
        for instrument in instruments:
            fileName = os.path.join(storage, "%s-%d-netease.csv" % (instrument, year))
