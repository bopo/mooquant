...
class BaseBarFeed(feed.BaseFeed):
    """Base class for :class:`mooquant.bar.Bar` providing feeds.

    :param frequency: The bars frequency. Valid values defined in :class:`mooquant.bar.Frequency`.
    ...
    # Subclasses should implement this and return a mooquant.bar.Bars or None if there are no bars.
    @abc.abstractmethod
    def getNextBars(self):
        """Override to return the next :class:`mooquant.bar.Bars` in the feed or None if there are no bars.
    ...
    def getCurrentBars(self):
        """Returns the current :class:`mooquant.bar.Bars`."""
        return self.__currentBars

    def getLastBar(self, instrument):
        """Returns the last :class:`mooquant.bar.Bar` for a given instrument, or None."""
    ...
    def getDataSeries(self, instrument=None):
        """Returns the :class:`mooquant.dataseries.bards.BarDataSeries` for a given instrument.

        :param instrument: Instrument identifier. If None, the default instrument is returned.
        :type instrument: string.
        :rtype: :class:`mooquant.dataseries.bards.BarDataSeries`.
        """
